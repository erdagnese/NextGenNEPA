...
metadata <- read.csv(here("Output", "COI.all.metadata.csv"))
ASV.table <- read.csv(here("Output", "COI.merged.ASV.table.csv"))
...
# metadata %>%
#   mutate(Index.number = case_when(str_detect(Index.number, "[0-9]{1}$")       ~     str_replace(Index.number, "Tag_", "Tag_0"),
#                          TRUE                                  ~     Index.number  )) -> metadata
# metadata %>% mutate(x= str_count(sample_id, pattern = "_")) %>% arrange(desc(x)) 
   
# ASV.table %>% distinct(Sample_name) %>% mutate(x= str_count(Sample_name, pattern = "_")) %>% arrange(desc(x)) # Also in the ASV table
# # Fix it here
...
```{r split into two}
# first we need to rename the ASV table column from Sample_name to Sample.number
...
ASV.nested %>% 
  transmute(Summary = map(sample, ~ how.many(ASVtable = .,round = 0)))  -> ASV.summary
```
